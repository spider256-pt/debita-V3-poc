...
contract Voter is IVoter, ERC2771Context, ReentrancyGuard {
    ...
    function killGauge(address _gauge) external {
        ...
        if (_claimable > 0) {
            IERC20(rewardToken).transfer(minter, _claimable);
    ...
    function notifyRewardAmount(uint256 _amount) external {
        ...
        IERC20(rewardToken).transferFrom(sender, address(this), _amount); // transfer the distribution in
        ...
        if (_supplied > 0) {
            ...
            if (_delta > 0) {
                ...
                if (isAlive[_gauge]) {
                    ...
                    IERC20(rewardToken).transfer(minter, _share); // send rewards back to Minter so they're not stuck in Voter
                }
            }
        } else {
