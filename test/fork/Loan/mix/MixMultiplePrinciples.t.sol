...
contract testMultiplePrinciples is Test {
    ...
    function testCheckFees() public {
        // borrow info
        DBOImplementation.BorrowInfo memory borrowInfo = BorrowOrder.getBorrowInfo();
        uint256 AEROAmount = 25e17;
        uint256 wETHAmount = 58e17;

        uint256 balanceBefore = AEROContract.balanceOf(feeAddress);
        uint256 balanceBeforeConnector = AEROContract.balanceOf(connector);
        uint256 balanceBeforeWETH = wETHContract.balanceOf(feeAddress);
        uint256 balanceBeforeWETHConnector = wETHContract.balanceOf(connector);
        matchOffers();

        uint256 balanceAfter = AEROContract.balanceOf(feeAddress);
        uint256 balanceAfterConnector = AEROContract.balanceOf(connector);
        uint256 balanceAfterWETH = wETHContract.balanceOf(feeAddress);
        uint256 balanceAfterWETHConnector = wETHContract.balanceOf(connector);

        uint256 duration = borrowInfo.duration;

        uint256 feePercentage = (duration * 4) / 86400;
        if (feePercentage > 80) {
            feePercentage = 80;
        }
        if (feePercentage < 20) {
        ...
        uint256 fee = (AEROAmount * feePercentage) / 10000;
        uint256 feeWETH = (wETHAmount * feePercentage) / 10000;

        uint256 feeConnectorWETH = (feeWETH * 1500) / 10000;
        uint256 feeConnector = (fee * 1500) / 10000;

        assertEq(balanceBefore + fee - feeConnector, balanceAfter, "Fee Debita");
        assertEq(balanceBeforeConnector + feeConnector, balanceAfterConnector, "Fee Connector");

        assertEq(balanceBeforeWETH + feeWETH - feeConnectorWETH, balanceAfterWETH, "Fee Debita WETH");

        assertEq(balanceBeforeWETHConnector + feeConnectorWETH, balanceAfterWETHConnector, "Fee Connector WETH");

        vm.startPrank(borrower);

        // extendLoan
        vm.warp(block.timestamp + 86401);
        uint256 balanceBeforeExtend = AEROContract.balanceOf(feeAddress);
        uint256 balanceWETHBeforeExtend = wETHContract.balanceOf(feeAddress);
        wETHContract.approve(address(DebitaV3LoanContract), 100e18);
        AEROContract.approve(address(DebitaV3LoanContract), 100e18);
        uint256 interest = calculateInterest(0);
        uint256 interestSecondOffer = calculateInterest(1);
        uint256 interestThirdOffer = calculateInterest(2);
        DebitaV3LoanContract.extendLoan();

        uint256 feeInterest = (interest * 1500) / 10000;
        uint256 feeInterestSecondOffer = (interestSecondOffer * 1500) / 10000;
        uint256 feeInterestThirdOffer = (interestThirdOffer * 1500) / 10000;

        uint256 balanceAfterExtend = AEROContract.balanceOf(feeAddress);
        uint256 balanceWETHAfterExtend = wETHContract.balanceOf(feeAddress);
        vm.stopPrank();
        uint256 feePercentageExtend = 80 - feePercentage;
        uint256 balanceBeforeFirstLender = AEROContract.balanceOf(firstLender);
        ...
        vm.stopPrank();
        vm.startPrank(borrower);
        uint256 feeExtend = (AEROAmount * feePercentageExtend) / 10000;
        uint256 feeExtendWETH = (wETHAmount * feePercentageExtend) / 10000;

        vm.expectRevert();
        DebitaV3LoanContract.extendLoan();
        vm.stopPrank();
        assertEq(balanceAfterFirstLender, balanceBeforeFirstLender + interest - feeInterest, "First lender balance");
        /*  assertEq(
        ...
        assertEq(
            balanceWETHBeforeExtend + feeExtendWETH + feeInterestSecondOffer + feeInterestThirdOffer,
    ...
    function testClaimDebtAfterExtend() public {
        ...
        wETHContract.approve(address(DebitaV3LoanContract), 100e18);
        vm.warp(block.timestamp + 86401);
        uint256 interestFirstOffer = calculateInterest(0);
        uint256 interestSecondOffer = calculateInterest(1);
        uint256 interestThirdOffer = calculateInterest(2);

        uint256 fee = (interestFirstOffer * 1500) / 10000;
        uint256 feeSecondOffer = (interestSecondOffer * 1500) / 10000;
        uint256 feeThirdOffer = (interestThirdOffer * 1500) / 10000;
        ...
        // claim debt again
        vm.startPrank(firstLender);
        uint256 balanceBefore_AfterPayment = AEROContract.balanceOf(firstLender);
        uint256 interestFirstOffer_AfterPayment = calculateInterest(0);
        uint256 fee_AfterPayment = (interestFirstOffer_AfterPayment * 1500) / 10000;
        DebitaV3LoanContract.claimDebt(0);
        uint256 balanceAfter_AfterPayment = AEROContract.balanceOf(firstLender);
        vm.stopPrank();

        vm.startPrank(secondLender);
        uint256 balanceBeforeSecond_AfterPayment = wETHContract.balanceOf(secondLender);
        DebitaV3LoanContract.claimDebt(1);
        uint256 balanceAfterSecond_AfterPayment = wETHContract.balanceOf(secondLender);
        vm.stopPrank();

        vm.startPrank(thirdLender);
        uint256 balanceBeforeThird_AfterPayment = wETHContract.balanceOf(thirdLender);
        DebitaV3LoanContract.claimDebt(2);
        uint256 balanceAfterThird_AfterPayment = wETHContract.balanceOf(thirdLender);
        vm.stopPrank();

        assertEq(balanceAfterFirstLender, balanceBeforeFirstLender + interestFirstOffer - fee, "First lender balance");
        ...
        assertEq(
            ...
            balanceBefore_AfterPayment + interestFirstOffer_AfterPayment - fee_AfterPayment + 25e17,
